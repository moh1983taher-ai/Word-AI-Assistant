...
async function analyzeReferencesWithAI(materials) {
    ...
    // =====================================================
    // 2. تجهيز المواد الخام للذكاء الاصطناعي
    //
    // نرسل جميع المواد دفعة واحدة.
    //
    // لا نقوم هنا:
    // - بتصنيف المرجع
    // - بدمج المراجع
    ...
    // - بفصل المراجع يدويًا
    //
    // الذكاء الاصطناعي هو المسؤول عن ذلك كله.
    // =====================================================
...
المواد مرتبة بحسب موضع ظهورها في المستند.

مهمتك تنفيذ العملية الكاملة في طلب واحد فقط.

لا تعِد بنتائج تحليل أولي منفصلة ثم نتائج دمج لاحقة.
بل ابنِ مباشرة القائمة النهائية الموحدة للمراجع.
...
multiple-reference
المادة تحتوي أكثر من مرجع مستقل.

ibid
المصدر نفسه / المرجع نفسه أو إحالة تعتمد على مرجع سابق.
...
=====================================================
ثانيًا: فصل المراجع المتعددة
=====================================================

هذه قاعدة أساسية جدًا.

إذا احتوت مادة واحدة على أكثر من مرجع مستقل،
فيجب فصلها إلى مراجع مستقلة قبل الدمج.
...
ولا يجوز وضع المرجعين في سجل واحد.
...
كل مرجع مستقل من المادة يجب أن يصبح عنصرًا مستقلاً
في القائمة النهائية إذا لم يكن له سجل سابق مطابق.

=====================================================
ثالثًا: وحدة الدمج
=====================================================

وحدة الدمج هي "المرجع" وليس "المادة".

إذا كانت المادة تحتوي على:

مرجع A + مرجع B

فلا يجوز إنشاء سجل واحد لهما.

بعد فصل A وB:

- إذا ظهر A لاحقًا مرة أخرى، يدمج مع A.
- إذا ظهر B لاحقًا مرة أخرى، يدمج مع B.

=====================================================
رابعًا: توحيد المراجع
=====================================================

ادمج المرجع نفسه إذا ظهر بصيغ مختلفة.

مثال:

الزركشي، البحر المحيط، 8/85
انظر: الزركشي، البحر المحيط في أصول الفقه، 8/85
الزركشي، البحر المحيط 8/85

هذه كلها مرجع واحد.

لكن لا تدمج كتابين مختلفين لمجرد تشابه الاسم.
...
- "المصدر نفسه"
- ترتيب الاسم والكتاب
- وجود بيانات نشر في صيغة وعدم وجودها في أخرى

لا يجعل المرجع مرجعًا جديدًا.

=====================================================
خامسًا: حل المصدر نفسه
=====================================================
...
يجب الرجوع إلى آخر مرجع مناسب سابقًا في ترتيب المواد
وحل الإحالة عليه.
...
يجب أن تكون النتيجة سجل النووي نفسه مع موقعين:

11/417
11/415

ولا تنشئ سجلًا اسمه "المصدر نفسه".

إذا تعذر تحديد المرجع السابق بصورة موثوقة:
احتفظ بالمادة في occurrences الخاصة بالمادة المناسبة
واجعل needsReview = true
وأضف ملاحظة تشرح سبب الشك.
...
هذه ليست مراجع خارجية إذا دل السياق على أنها إحالات
إلى صفحات الدراسة الحالية.

لا تدخل في references النهائية.
...
لا تحولها إلى كتاب عادي.

يمكن إنشاء سجل مستقل لها بنوع:
...
إذا كانت المادة تحتوي شرحًا + مرجعًا:
استخرج المرجع منها أيضًا.
...
يجب استخراج مرجع:
الفراهيدي - العين - 5/253

مع الاحتفاظ بنص المادة الأصلي.
...
إذا كان المؤلف مكتوبًا:
احتفظ به.

إذا كان العنوان مكتوبًا:
احتفظ به.

إذا كان أحدهما ناقصًا:
اتركه فارغًا.

يمكن ربط اسم مختصر بمرجع سابق كامل إذا كان السياق
والترتيب يسمحان بذلك بدرجة عالية من الثقة.
...
يمكن اعتبارهما المرجع نفسه إذا كان السياق السابق
يوفر أساسًا واضحًا لذلك.

لكن:
"المنخول 454"

لا تفترض المؤلف تلقائيًا إلا إذا كان السياق السابق
يثبت أنه الغزالي بدرجة عالية.

=====================================================
عاشرًا: الأخطاء المطبعية
=====================================================
...
فاحتفظ بالصيغة الأصلية في variants.

إذا كان واضحًا أنها قد تكون خطأ:
ضع needsReview = true
وأضف ملاحظة.
...
اجمع جميع مواضع الاستشهاد للمرجع نفسه.
...
يجب أن يكون سجلًا واحدًا وله ثلاثة locations.

لا تكرر location نفسها إذا كانت مكررة حرفيًا.
...
كلها variants للمرجع نفسه.

لا تكرر النص نفسه حرفيًا أكثر من مرة.

=====================================================
الثالث عشر: occurrences
=====================================================

كل ظهور فعلي للمرجع يجب تسجيله.
...
إذا احتوت المادة الواحدة على مرجعين:
يجوز أن يشترك المرجعان في materialId نفسه،
لأنهما ظهرا داخل المادة الأصلية نفسها.
...
استخدم الثقة العالية عندما تكون هوية المرجع واضحة.

استخدم ثقة أقل عند:
- نقص المؤلف
- نقص العنوان
- احتمال خطأ في الجزء
- احتمال اختلاف كتابين
- حل غير مؤكد للمصدر نفسه
- دمج غير مؤكد

needsReview = true عند وجود شك حقيقي.

=====================================================
الخامس عشر: النتيجة النهائية
=====================================================
...
قواعد حاسمة قبل إخراج النتيجة:

1. references = المراجع النهائية الموحدة فقط.
2. لا تنشئ سجلًا منفصلًا لكل ظهور.
3. لا تضع مرجعين مستقلين في سجل واحد.
4. افصل المراجع المتعددة أولًا ثم نفذ الدمج.
5. حل المصدر نفسه قبل إخراج القائمة النهائية.
6. لا تجعل variants بديلًا عن فصل المراجع المستقلة.
7. عدد occurrences يجب أن يمثل عدد الظهور الحقيقي للمرجع.
8. اجمع locations للمرجع نفسه.
9. احتفظ بالصيغة الأصلية في variants.
10. لا تخترع معلومات غير موجودة.
11. لا تحذف مادة غامضة؛ اجعل الحالة review عند الحاجة.
12. حافظ على ترتيب أول ظهور للمرجع في القائمة النهائية قدر الإمكان.
`;
    ...
    const userPrompt =
        [
            ...
            "",
            "تذكّر أن المادة الواحدة قد تحتوي على عدة مراجع مستقلة.",
            "افصلها أولًا، ثم ادمج كل مرجع مع ظهوره الآخر.",
            "",
            JSON.stringify(
                payload,
                null,
