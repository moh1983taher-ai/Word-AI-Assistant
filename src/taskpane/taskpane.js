...
async function buildStreamingContext(
    ...
    if (
        ...
        // بناء خريطة الإحالات
        //
        // هذه الخريطة تخبر الذكاء الاصطناعي
        // ما هي أرقام المقاطع المسموح بها
        // وما الذي يشير إليه كل رقم.
        // ==================================
        ...
        // ==================================
        // قائمة أرقام الإحالات المسموح بها
        // ==================================

        const allowedCitationRanks =
            citationSources
                .map(
                    function (
                        source
        ...
        const allowedCitationText =
            allowedCitationRanks.length >
                0

                ? allowedCitationRanks
                    .map(
                        function (
                            rank
                        ) {
        ...
        // ==================================
        // تجهيز المحتوى الذي سيرسل للـAI
        // ==================================

        userContent =
            [

                "أنت تجيب عن سؤال مستخدم داخل أداة بحث أكاديمية.",
                ...
                "مصطلحات السؤال: " +
                    (
                        Array.isArray(
                            documentContext.matchedTerms
                        ) &&
                        documentContext.matchedTerms.length >
                            0
                ...
                citationMap ||
                    "لا توجد إحالات متاحة.",

                "",

                "أرقام الإحالات التي يمكن استخدامها فقط:",
                ...
                "محرك الاسترجاع قدم لك أفضل المقاطع المتاحة، ولا يعني ذلك أن المستند يحتوي بالضرورة على جواب كامل للسؤال.",

                "حلل المادة المستخرجة ثم صغ منها إجابة مترابطة ومباشرة.",

                "لا تحاول إنشاء معلومات غير موجودة في المادة المستخرجة.",

                "إذا كان جزء من السؤال غير مدعوم بالمادة المستخرجة، صرّح بذلك بوضوح.",

                "لا تستخدم المعرفة العامة لسد نقص المستند إلا إذا طلب المستخدم ذلك صراحة.",

                "وجود مصطلح من مصطلحات السؤال داخل مقطع لا يعني أن المقطع يجيب عن السؤال؛ استخدمه فقط عندما يكون ذا صلة حقيقية.",

                "ادمج الأفكار المتشابهة ولا تكررها.",

                "لا تحول كل مقطع إلى فقرة مستقلة.",

                "رتب الإجابة بحسب محاور السؤال عند الحاجة.",

                "إذا كان السؤال يتطلب مقارنة أو علاقة بين مفهومين، استخرج من المادة ما يمكن أن يثبت العلاقة، وإن لم تكن المادة كافية فلا تخترع العلاقة.",

                "ضع الإحالة مباشرة بعد الفكرة التي يدعمها المصدر.",

                "استخدم صيغة [مقطع X] فقط.",

                "X يجب أن يكون واحدًا من أرقام الإحالات المتاحة أعلاه.",

                "لا تخترع رقم مقطع.",

                "لا تغير رقم المقطع.",

                "لا تستخدم إحالة إذا كان المقطع لا يدعم الفكرة المنسوبة إليه.",

                "إذا دعمت الفكرة نفسها عدة مقاطع، يجوز جمع الإحالات بعد الفكرة.",

                "لا تكرر الإحالة نفسها دون حاجة.",
