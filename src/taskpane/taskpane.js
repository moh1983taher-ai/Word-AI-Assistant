...
نفذ العملية الكاملة في طلب واحد فقط.

لا تُخرج تحليلًا أوليًا منفصلًا ثم مرحلة دمج لاحقة.
بل ابنِ مباشرة قائمة المراجع النهائية الموحدة.
...
فافصلها إلى سجلات مستقلة.

مثال:

"الشيرازي، اللمع 37، السمعاني، قواطع الأدلة، 1/193"

يجب أن ينتج عنها مرجعان مستقلان:

الشيرازي - اللمع - ص 37

السمعاني - قواطع الأدلة - 1/193

ولا يجوز وضعهما في سجل واحد.

=====================================================
التوحيد
=====================================================

ادمج المرجع نفسه عند اختلاف الصياغة أو الصفحة أو وجود
"انظر" أو "ينظر" أو "المصدر نفسه".
...
"المصدر نفسه" و"المرجع نفسه" ليسا مرجعًا جديدًا.

اربطهما بالمرجع السابق المناسب.

إذا تعذر الحسم، استخدم needsReview = true.
...
لا تدخل في قائمة references النهائية إذا كانت إحالة
...
الشرح وحده لا يدخل في references.

الشرح الذي يحتوي مرجعًا يجب استخراج المرجع منه.

=====================================================
المعلومات
=====================================================

لا تخترع مؤلفًا أو عنوانًا أو ناشرًا أو سنة نشر.

إذا كانت المعلومة غير موجودة فاتركها فارغة.

يمكن الاستفادة من السياق السابق عند التأكد بدرجة عالية.

=====================================================
locations
=====================================================

اجمع جميع الأجزاء والصفحات الخاصة بالمرجع نفسه.
...
احتفظ بالصيغ الأصلية المختلفة للمرجع نفسه.

لا تكرر الصيغة نفسها.

=====================================================
occurrences
=====================================================

كل ظهور فعلي للمرجع يسجل داخل occurrences.
...
- references تحتوي المراجع النهائية الموحدة فقط.
- لا تنشئ سجلًا منفصلًا لكل ظهور.
- لا تضع مرجعين مستقلين في سجل واحد.
- افصل المراجع المتعددة أولًا.
- حل المصدر نفسه.
- اجمع locations.
- اجمع variants.
- اجمع occurrences.
- لا تخترع معلومات.
- حافظ على ترتيب أول ظهور للمرجع.
`;
    ...
    if (
        ...
        // مهم:
        // نعيد الكائن كاملًا: references + stats
