...
//    → لا يتم استدعاء محرك المستند.
//    → AI يجيب عن السؤال مباشرة.
//
// 2) دردشة المستند:
//    documentEnabled = true
//    → يتم استدعاء Orama + سياق المستند.
//    → AI يجيب اعتمادًا على المادة المستخرجة.
//
// وجود currentDocument وحده لا يفعّل البحث.
// =====================================================
...
async function buildStreamingContext(
    ...
    // ==================================
    // هل المستند مفعّل لهذه المحادثة؟
    // ==================================

    const documentEnabled =
        Boolean(
            currentChat &&
            currentChat.documentEnabled ===
                true
        );


    // ==================================
    // السياق الخاص بالمستند
    // ==================================
    ...
    // ==================================
    // لا نستدعي البحث إلا عند التفعيل
    // ==================================
    ...
    if (
        ...
        userContent =
            [
                ...
                "أجب اعتمادًا على المادة المستخرجة من المستند بوصفها المصدر الأساسي.",
                ...
                "إذا لم تكف المادة للإجابة عن جزء من السؤال، صرّح بذلك بوضوح.",

                "لا تستخدم المعرفة العامة لسد نقص المستند إلا إذا طلب المستخدم ذلك صراحة.",

                "ضع الإحالة [مقطع X] بعد الفكرة التي يدعمها المقطع.",

                "استخدم أرقام المقاطع الموجودة في المادة فقط.",

                "لا تخترع إحالات.",

                "إذا دعمت عدة مقاطع الفكرة نفسها، اجمع الإحالات.",

                "قدم إجابة تركيبية مترابطة."
