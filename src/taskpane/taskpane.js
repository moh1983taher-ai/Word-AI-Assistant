...
function splitReferenceMaterialsIntoBatches(
    materials,
    maxBatchSize
) {
        ...
        async function () {
            ...
            try {
                ...
                for (
                    ...
                    const batches =
                        splitReferenceMaterialsIntoBatches(
                            processedReferences,
...
// =====================================================
// تنظيف نصوص نتائج المكتبة
// خاص بنتائج المكتبة فقط
// =====================================================
...
function cleanLibraryText(text) {
    ...
    // -------------------------------------------------
    // تحويل الأقواس المزدوجة الخاصة بالمراجع
    // (( ... )) ← ( ... )
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // إزالة بعض شوائب OCR المعروفة في بداية النص
    // مثل:
    // e Y \
    // Yo
    // وغيرها من الرموز المنفصلة
    // -------------------------------------------------

    value =
        value.replace(
            /^\s*(?:e\s*[Yy]\s*\\+|Yo|YO|Y\s*\\+)\s*/i,
            ""
        );


    // -------------------------------------------------
    // إزالة الرموز الغريبة المنفردة في البداية
    // مع الحفاظ على الحروف العربية
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // إزالة بقايا OCR قبل أول حرف عربي
    // فقط إذا كانت البداية عبارة عن رموز لاتينية/
    // علامات، وليست كلمة مفيدة
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // تقليل تكرار علامات الترقيم المشوهة
    // -------------------------------------------------
...
ستستلم مواد مستخرجة من مستند Word مرتبة بحسب ظهورها.

مهمتك:
تحليل المراجع الواردة في المواد، وفصل المراجع المختلفة،
وتوحيد المرجع نفسه عند تكراره، وربط كل ظهور بالمادة الأصلية.

=====================================================
1. هوية المادة الأصلية
=====================================================

قد تبدأ بعض المواد بعلامة تقنية مثل:
...
هذه العلامة ليست من النص الأصلي، وإنما هي هوية ثابتة للمادة.

إذا كانت المادة تحمل:

【FOOTNOTE_ID:footnote-4】

فإن كل occurrence ناتج عن مرجع موجود داخل هذه المادة
يجب أن يحمل:
...
لا تغيّر هذه الهوية ولا تستبدلها.

إذا احتوت المادة نفسها على عدة مراجع مستقلة،
فكل مرجع يحصل على occurrence مستقل،
لكن جميعها تشترك في materialId نفسه.

=====================================================
1.1 مصدر المادة ودرجة الثقة
=====================================================

المواد القادمة من المصادر المختلفة ليست متساوية في الدلالة.

إذا كان source = "footnote" أو source = "endnote":
فتُعد المادة مصدرًا أساسيًا لاستخراج المراجع.

إذا كان source = "main-text":
فتُعد المادة مرشحًا لاستخراج التوثيق، وليست مرجعًا تلقائيًا.

لا تعتبر مادة المتن مرجعًا لمجرد وجودها بين قوسين،
أو احتوائها على أرقام، أو فواصل، أو صياغة يمكن أن تشبه عنوانًا.

لا تُسجل مادة المتن كمرجع إلا إذا ظهرت فيها قرينة توثيقية واضحة،
مثل وجود إحالة ببليوغرافية صريحة، أو موضع استشهاد مرتبط بمصدر،
أو صيغة توثيق معروفة تدل بوضوح على الإحالة إلى مصدر خارجي.

لا تُنشئ مرجعًا من مادة المتن إذا كانت طبيعتها الظاهرة
شرحًا، أو مثالًا، أو شاهدًا لغويًا، أو مصطلحًا، أو كلمة،
أو تركيبًا، أو عبارة تعليمية، أو نصًا قرآنيًا.

إذا كان احتمال كون مادة المتن توثيقًا ضعيفًا،
فلا تنشئ منها reference.

=====================================================
1.2 الإحالات القرآنية
=====================================================

الإحالة القرآنية ليست مرجعًا ببليوغرافيًا.

لا تُنشئ reference مستقلًا من أي إحالة إلى القرآن.

إذا اجتمعت إحالة قرآنية مع مرجع ببليوغرافي داخل المادة نفسها،
فافصل بينهما، ولا تجعل الإحالة القرآنية جزءًا من عنوان المرجع
أو مؤلفه أو بياناته.

=====================================================
1.3 المواد غير المرجعية
=====================================================

إذا كانت المادة شرحًا أو مثالًا أو شاهدًا أو مصطلحًا أو كلمة
أو تركيبًا أو عبارة تعليمية، فلا تُنشئ لها reference.

إذا احتوت المادة على شرح يتضمن مرجعًا خارجيًا واضحًا،
استخرج المرجع الخارجي فقط.

إذا لم يظهر فيها مرجع خارجي واضح، فلا تُنشئ أي reference.

=====================================================
1.4 التعامل مع الدفعات
=====================================================

قد تصل المادة إلى النموذج ضمن دفعة مستقلة عن بقية مواد المستند.

لذلك لا تستنتج هوية المرجع من ترتيب المواد وحده،
ولا تفترض أن مادة قريبة من مادة أخرى تنتمي إليها.

حلل كل مادة وفق نصها الأصلي وsource والبيانات المرفقة بها.
...
هوية المرجع تعتمد أساسًا على عنوان الكتاب أو المصدر،
ومع المؤلف إذا كان مذكورًا في المادة الأصلية.

إذا كان المؤلف نفسه له كتب مختلفة، فكل كتاب مرجع مستقل،
ولا تدمج الكتب المختلفة للمؤلف نفسه.

إذا جاء اسم المؤلف ملتصقًا بعنوان الكتاب في صيغة نسبة إليه،
فافصل اسم المؤلف عن العنوان واحذف صيغة النسبة من العنوان.

أما إذا كان الاسم جزءًا أصيلًا من عنوان الكتاب، فلا تفصله.

إذا كان للعنوان كتاب مستقل وشرح أو حاشية أو تعليق أو تهذيب،
فلا تدمج بينها لمجرد التشابه في العنوان.
...
لا تضف اسم مؤلف إلا إذا كانت نسبة الكتاب إليه معلومة وموثوقة،
ولا تُنشئ اسم المؤلف أو تنسب الكتاب إليه بالاستنتاج أو التخمين.

اعتمد فقط على المعلومات الموثوقة التي لديك عن الكتاب.

إذا كان المؤلف مذكورًا في المادة الأصلية بوضوح، فاحتفظ به.

لا تُنسب الكتب إلى مؤلفين بالاستنتاج؛ نسبة الكتاب إلى مؤلفه
معلومة ببليوغرافية يجب أن تكون ثابتة وموثوقة.

${addMissingAuthors
    ? `
=====================================================
3.1 تقصي أسماء المؤلفين
=====================================================

عند تفعيل هذا الخيار، استعن بمعرفتك الببليوغرافية الموثوقة
لتحديد مؤلف الكتاب غير المذكور في الحاشية.

حدّد الكتاب المقصود أولًا، ثم انسبه إلى مؤلفه الصحيح.

استخدم اسم الشهرة العلمي المختصر المناسب للتوثيق.

لا تضع اسمًا إلا إذا كانت نسبة الكتاب إلى مؤلفه معلومة وموثوقة،
وإلا فاترك author فارغًا.
`
    : ``
}
...
لا تكتفِ بأول مرجع تتعرف عليه،
ولا تتوقف بعد استخراج بعض المراجع.

إذا ظهر اسم كتاب أو مصدر ومعه موضع استشهاد واضح،
فهو مرجع صالح للاستخراج حتى لو لم يمكن تحديد مؤلفه.

إذا كان المرجع ظاهرًا بوضوح في النص،
فلا يجوز حذفه بسبب نقص المعلومات.

ولا تنشئ مرجعًا غير موجود في النص الأصلي.

إذا احتوى المرجع نفسه على أكثر من موضع،
فاحتفظ بجميع مواضعه داخل locations.
...
جميع occurrences الناتجة من المادة نفسها تحمل
materialId نفسه.

إذا ظهر المرجع نفسه في حاشيتين مختلفتين،
فهذا ظهوران مستقلان ويجب الاحتفاظ بهما.

توحيد المرجع لا يعني حذف مرات ظهوره.
...
احفظ موضع الاستشهاد كما ورد في المادة الأصلية.

إذا كان المرجع مصحوبًا بمجلد وصفحة،
فاحفظ المجلد والصفحة.

إذا كان مصحوبًا بنطاق صفحات،
فاحفظ النطاق كاملًا.

إذا كان المرجع نفسه مذكورًا مع أكثر من موضع،
فاحتفظ بجميع المواضع الخاصة به داخل locations.

لا تحذف موضعًا من مواضع المرجع نفسه.

لا تنقل موضعًا إلى مرجع آخر.

لا تختصر نطاق الصفحات إلى آخر رقم فقط.

=====================================================
7. المصدر نفسه
=====================================================

"المصدر نفسه" ليس كتابًا جديدًا.

إذا كان واضحًا أنه يعود إلى المرجع السابق،
اربطه بذلك المرجع.

إذا كان بلا موضع أو سياق كافٍ،
لا تخترع مرجعًا.

إذا كان معه موضع واضح،
اربطه بالمرجع السابق المناسب وسجل موضعه.

إذا كان "المصدر نفسه" بلا رقم أو موضع واضح،
لا تنشئ له occurrence مستقلًا.

=====================================================
8. الإحالات الداخلية
=====================================================

إذا كانت العبارة مجرد إحالة إلى صفحات الدراسة الحالية،
فهي ليست مرجعًا خارجيًا.

لا تحول الإحالة الداخلية إلى مرجع خارجي.
...
variants تستخدم للصيغ المختلفة للمرجع نفسه فقط.

لا تستخدم variants لدمج كتب مختلفة.

كل صيغة مختلفة يجب أن تعود إلى المرجع نفسه،
ولا يجوز وضع كتاب مختلف داخل variants.

=====================================================
11. الدقة وعدم الحذف
=====================================================

كن حريصًا جدًا على عدم إسقاط المراجع.

إذا كان المرجع ظاهرًا بوضوح في النص الأصلي،
فلا تحذفه.

إذا كان عنوان المرجع واضحًا والموضع واضحًا،
فيجب الاحتفاظ به حتى لو كان المؤلف غير معروف.

إذا كانت معلومات المرجع ناقصة:
احتفظ بما هو موجود،
...
لا تدمج كتبًا مختلفة للمؤلف نفسه.
...
function sortUnifiedReferences(
    references,
    order
) {
...
async function applyFootnoteSuggestions(
    ...
    /*
     * =========================================================
     * 7. القاعدة الإضافية الوحيدة:
     *
     * إذا لم يوجد العنوان الكامل، نبحث عن بداية العنوان
     * المختصرة الموجودة فعليًا في الحاشية.
     *
     * مثال:
     *
     * title:
     ...
     * نلتقط "المجموع".
     *
     * لا تُستخدم هذه القاعدة إذا وجد العنوان الكامل.
     * =========================================================
     */

    function findShortTitleIdentity(
        text,
        reference
    ) {
        ...
        /*
         * إذا كان العنوان كلمة واحدة فقط،
         * فلا توجد صيغة مختصرة إضافية.
         */
        if (
            words.length < 2
        ) {
    ...
    /*
     * =========================================================
     * 8. استخراج هوية المرجع من suggestedText
     * =========================================================
     */
    ...
    /*
     * =========================================================
     * 9. توسيع المطابقة المختصرة إلى المؤلف
     *
     * هذه لا تعمل إلا عندما نستخدم القاعدة الجديدة
     * الخاصة بالعنوان المختصر.
     *
     * مثال:
     *
     * النووي، المجموع
     ...
     * النووي، المجموع
     *
     * ثم نستبدل الكل بالهوية الجديدة.
     * =========================================================
     */

    function expandShortMatchToAuthor(
        text,
        match,
        author
    ) {
        ...
        const start =
            beforeMap.normalized.length -
            authorNormalized.length;

        if (
            start < 0 ||
            start >=
                beforeMap.rawMap.length
        ) {
    ...
    /*
     * =========================================================
     * 10. الحصول على عناصر الحواشي باستخدام w:id الحقيقي
     * =========================================================
     */
    ...
    /*
     * =========================================================
     * 11. استخراج النص المرئي للحاشية
     * =========================================================
     */
    ...
    /*
     * =========================================================
     * 12. استبدال جزء من النص مع الحفاظ على بنية XML
     * =========================================================
     */
    ...
    /*
     * =========================================================
     * 13. معالجة footnotes.xml / endnotes.xml
     * =========================================================
     */

    async function processNotesPart(
        zip,
        path,
        localName,
        source
        ...
        for (
            ...
            referenceGroups.forEach(
                function (
                    ...
                    /*
                     * أولًا: السلوك القديم تمامًا.
                     *
                     * إذا وجد العنوان الكامل أو إحدى صيغ الهوية
                     * الموجودة سابقًا، نستخدمها ولا ندخل القاعدة
                     * الجديدة إطلاقًا.
                     */
                    let identityMatch =
                        findBestIdentity(
                    ...
                    /*
                     * =================================================
                     * القاعدة الوحيدة الجديدة:
                     *
                     * إذا لم نجد أي تطابق بالطريقة القديمة،
                     * نجرب بداية العنوان فقط.
                     *
                     * مثال:
                     *
                     * العنوان:
                    ...
                    if (
                        ...
                        if (
                            ...
                            /*
                             * نضم المؤلف القديم إلى المطابقة المختصرة.
                             *
                             * مثال:
                             *
                             * النووي، المجموع
                             ...
                             */
                            identityMatch =
                                expandShortMatchToAuthor(
                                    originalText,
                                    identityMatch,
                                    String(
                                        group.reference?.author ||
                                        ""
                                    ).trim()
                                );
    ...
    const totalApplied =
        Number(
            footnoteResult?.applied ||
            0
        ) +
        Number(
            endnoteResult?.applied ||
            0
        );

    const totalSkipped =
        Number(
            footnoteResult?.skipped ||
            0
        ) +
        Number(
            endnoteResult?.skipped ||
            0
        );
    ...
    /*
     * =========================================================
     * 17. توليد DOCX المعدل مرة واحدة
     * =========================================================
     */
