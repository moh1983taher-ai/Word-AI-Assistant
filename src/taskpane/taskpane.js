...
async function analyzeReferencesWithAI(
    ...
    // ============================================================
    // 3. تجهيز المواد بصورة مختصرة
    //
    // لا نرسل kind لأن الذكاء الاصطناعي سيحكم بنفسه.
    // لا نرسل context الطويل إلا عند الحاجة.
    // ============================================================
    ...
    // 4. التعليمات الموحدة
    //
    // الذكاء الاصطناعي ينفذ العملية كلها في طلب واحد:
    //
    // قراءة المواد
    // ↓
    // فهمها
...
أمامك جميع المواد المستخرجة من مستند Word دفعة واحدة.

مهمتك الأساسية:
بناء قائمة نهائية موحدة للمراجع الموجودة في المستند.

لا توجد مرحلة لاحقة لدمج النتائج.
أنت المسؤول عن التوحيد والدمج النهائي.

============================================================
أولاً: تحليل المواد
============================================================

كل مادة قد تكون واحدًا من الآتي:

- reference
مرجع ببليوغرافي.

- multiple-reference
مادة تحتوي على أكثر من مرجع.

- ibid
المصدر نفسه أو المرجع نفسه أو إحالة تعتمد على المرجع السابق.
...
- explanatory
شرح أو تعليق لا يمثل مرجعًا.

- mixed
شرح أو اقتباس يحتوي داخله على مرجع.

- review
مادة لا يمكن حسمها بثقة.

============================================================
ثانيًا: قواعد مهمة جدًا
============================================================

1. لا تعتمد على شكل الأقواس أو المعكوفات أو علامات الترقيم.

2. لا تعتبر كل رقم مرجعًا.

3. لا تخترع اسم مؤلف.

4. لا تخترع عنوان كتاب.

5. لا تخترع بيانات نشر أو طبعة أو سنة.

6. إذا كان المؤلف غير موجود في النص فاتركه فارغًا.

7. إذا كان العنوان معروفًا والمؤلف غير معروف فاترك المؤلف فارغًا.
...
أو:
"ص 67، 104"
وكان واضحًا أنها إحالة داخلية للدراسة، فلا تجعلها مرجعًا.

9. "المصدر نفسه" و"المرجع نفسه" لا ينشئان مرجعًا جديدًا.

10. يجب حل "المصدر نفسه" بالرجوع إلى أقرب مرجع سابق مناسب في ترتيب المواد.

11. إذا كانت مادة واحدة تحتوي أكثر من مرجع، افصل المراجع داخلها.

12. إذا تكرر المرجع نفسه بصيغ مختلفة، يجب أن يظهر في
سجل واحد فقط.
...
انظر: الزركشي، البحر المحيط 8/85

هذه كلها مرجع واحد.

13. عند الدمج:
اجمع جميع الصيغ المختلفة في variants.

14. اجمع جميع مواضع الاستشهاد المختلفة في locations.

15. occurrences يجب أن تحتوي على كل ظهور فعلي للمرجع.

16. لا تنشئ سجلًا مستقلًا لكل ظهور.

17. إذا ظهر المرجع نفسه 10 مرات:
يجب أن يكون له سجل واحد فقط و10 occurrences.

18. لا تدمج كتابين مختلفين لمجرد تشابه اسم المؤلف.

19. لا تدمج كتابين مختلفين لمجرد تشابه العنوان.

20. إذا كان الدمج غير مؤكد، اجعل needsReview=true.

21. لا تحذف مادة غامضة.

22. المواد غير المرجعية لا تظهر داخل references النهائية.

23. تخريج الحديث يمكن أن يكون سجلًا من نوع hadith.

24. journal وarticle وthesis تستخدم فقط عندما يكون ذلك واضحًا من المادة.

25. إذا كانت المادة شرحًا ثم احتوت على مرجع، استخرج المرجع داخل السجل المناسب.

26. لا تجعل نص الشرح نفسه عنوانًا للكتاب.

27. لا تجعل عبارة مثل:
"ت: 749 هـ"
أو:
"1997م"
مرجعًا مستقلاً.
...
إذا ظهر المرجع في أكثر من موضع:
اجمع المواضع كلها داخل locations.

============================================================
رابعًا: variants
============================================================

variants تحتوي على الصيغ الأصلية المختلفة التي ظهر بها المرجع.

لا تكرر الصيغة نفسها حرفيًا.

============================================================
خامسًا: occurrences
============================================================

كل ظهور للمرجع يجب تسجيله:
...
1.0
عندما يكون المرجع واضحًا جدًا.

0.9
عندما يكون المرجع واضحًا مع نقص بسيط.

0.8
عندما توجد قرائن جيدة لكن هناك احتمال خطأ.

أقل من ذلك عند وجود شك حقيقي.

needsReview=true عندما يكون هناك نقص أو غموض مؤثر.

============================================================
سابعًا: مهم جدًا
============================================================

لا تنشئ أكثر من سجل لنفس المرجع.
...
`;
    ...
    const userPrompt =
        [
            "حلل جميع المواد التالية وابنِ قائمة المراجع النهائية الموحدة:",
            "",
            JSON.stringify(
                payload,
                null,
    ...
    // ============================================================
        ...
        if (
            ...
            throw new Error(
                "أعاد نموذج الذكاء الاصطناعي استجابة فارغة."
            );
    ...
    // 8. توحيد شكل النتيجة
    //
    // parseUnifiedReferenceAIResult قد يعيد:
    //
    // { references: [...] }
    //
    // أو:
    ...
    // [...]
    //
    // لذلك نضمن أن هذه الدالة ترجع مصفوفة دائمًا.
    // ============================================================
    ...
    if (
        ...
        throw new Error(
            "لم يتم العثور على قائمة المراجع في نتيجة الذكاء الاصطناعي."
        );
    ...
    // ============================================================
    // 9. تنظيف السجلات النهائية دون إعادة تحليلها
    //
    // هذا ليس دمجًا جديدًا.
    // فقط حماية من البيانات الناقصة أو غير الصالحة.
    // ============================================================
                        ...
                        analyzeReferencesBtn.onclick =
                            async function (e) {
                                ...
                                try {
                                    ...
                                    //
                                    // هذه الدالة لا تقرر المرجع النهائي.
                                    // =================================================
                                    ...
                                    //
                                    // هذه هي المرحلة الوحيدة للتحليل.
                                    //
                                    // يجب أن تقوم الدالة:
                                    //
                                    // analyzeReferencesWithAI()
                                    //
                                    // بالتصنيف + استخراج المراجع +
                                    // فصل المراجع المتعددة +
                                    // حل المصدر نفسه +
                                    // استنتاج المؤلف والعنوان +
                                    // توحيد الصيغ +
                                    // دمج التكرارات +
                                    // جمع المواضع +
                                    // جمع صيغ الظهور.
                                    ...
                                    console.log(
                                        "السجلات النهائية من الذكاء الاصطناعي:",
                                        finalReferenceResults
                                    );


                                    // =================================================
                                    // 5. إحصاءات النتائج النهائية
                                    // =================================================
